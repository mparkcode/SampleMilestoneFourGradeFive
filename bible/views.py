...
def get_games(request):
    """Function to retrieve a list of games
    based on user input to the IGDB Search Form
    """
        ...
        for game in response.json():
            if game.get('first_release_date'):
                game['first_release_date'] = datetime.utcfromtimestamp(
                    int(game['first_release_date'])).strftime('%Y/%m/%d')
            cover_query = requests.get(
                ENDPOINT,
                headers={
                    'user-key': USER_KEY},
                params={
                    'fields': 'cover.*; where id = {}'.format(game['id'])})
...
def game_detail(request):
    """Function to retrieve an individual game
    based on user choice from get_games view
    """
    search_query = request.GET.get("query")
    if search_query:
        query = search_query
    ...
    if game_id:
        ...
        response = requests.get(
            ...
            headers={
                'user-key': USER_KEY},
            params={
                'fields': '*; where id = {}'.format(game_id)})
        ...
        if 'cover' in game:
            cover_query = requests.get(
                ...
                headers={
                    'user-key': USER_KEY},
                params={
                    'fields': 'cover.*; where id = {}'.format(game_id)})
            game['cover_url'] = cover_query.json(
            )[0]['cover']['url'].replace("t_thumb", "t_cover_big")
        ...
        if 'screenshots' in game:
            screenshot_query = requests.get(
                ENDPOINT,
                headers={
                    'user-key': USER_KEY},
                params={
                    'fields': 'screenshots.*; where id = {}'.format(game_id)})
