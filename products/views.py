...
def index(request):
    ...
    return render(request, "products/index.html",
                  {"games": games, 'article': article, 'form': form})
...
def show_consoles(request, brand):
    ...
    if search_query:
        ...
        consoles = Console.objects.filter(
            brand=brand).order_by('console_type')
        return render(request, "products/show_consoles.html",
                      {'consoles': consoles, 'brand': brand})
...
def show_games(request, console):
    ...
    if search_query:
        ...
        page = request.GET.get('page')
        games = paginator.get_page(page)
        return render(request, "products/show_games.html",
                      {"games": games, "console": console_type})
