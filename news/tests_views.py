...
class TestNewsViews(TestCase):
    ...
    def test_post_create_a_comment(self):
        ...
        response = self.client.post(
            "/news/{0}".format(article.id), {
                'content': "Example comment", 'comment_id': ""})
    ...
    def test_can_edit_comment(self):
        ...
        response = self.client.post(
            "/news/{0}".format(article.id), {
                'content': "Example comment", 'comment_id': comment.id})
        self.assertRedirects(response,
                             '/news/{0}'.format(article.id),
                             status_code=302,
                             target_status_code=200,
    ...
    def test_can_delete_comment(self):
        ...
        comment.save()
        response = self.client.post(
            "/news/{0}".format(article.id), {'comment_id': comment.id})
        self.assertRedirects(response,
                             '/news/{0}'.format(article.id),
                             status_code=302,
                             target_status_code=200,
    ...
    def test_comment_content_appears_in_form_to_edit(self):
        ...
        page = self.client.get(
            "/news/{0}?comment_id={1}".format(article.id, comment.id))
        self.assertEqual(page.status_code, 200)
        self.assertTemplateUsed(page, "news/view_article.html")
